...
```{r DataExercise-1a}
high.temp<-c(81,83,83,84,78,71,79,82,85,84)
low.temp<-c(55,56,58,60,61,63,63,64,64,64)
high.temp
low.temp
```
...
high.temp.celc<-my.function(high.temp)
low.temp.celc<-my.function(low.temp)
high.temp.celc
low.temp.celc

combined.temp<-data.frame(Low_Temp_F=low.temp, High_Temp_F=high.temp, Low_Temp_C=low.temp.celc, High_Temp_C=high.temp.celc)
combined.temp
```
...
```{r DataExercise-1d}
summary(combined.temp)
summary(combined.temp$Low_Temp_F)
sd(combined.temp$Low_Temp_F)
```
