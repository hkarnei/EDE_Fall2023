...
```{r project.setup, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}
#Import libraries
...
colnames(summary_PM25)[colnames(summary_PM25) == "mean_PM2.5"] <- '$\\mu g/m^3$'
knitr::kable(summary_PM25, caption = "Mean Particulates (2.5mm)")

colnames(summary_Ozone)[colnames(summary_Ozone) == "mean_Ozone"] <- '$\\mu g/m^3$'
...
```{r boxplot 1, warning=FALSE, echo=FALSE, fig.cap='Monthly Ozone, 2018', fig.width=6, fig.height=4}

data_2018 <- epa_data %>%
  filter(Year == 2018)

data_2018$month_abb <- month.abb[data_2018$Month]
data_2018$month_abb <- factor(data_2018$month_abb, levels = month.abb)

ggplot(data_2018, aes(x = month_abb, y =Ozone)) +
  geom_boxplot() +
  xlab('') +
...
```{r boxplot 2, warning=FALSE, echo=FALSE, fig.cap='Monthly Ozone, 2019', fig.width=6, fig.height=4}

p2 <- data_2019 <- epa_data %>%
  filter(Year == 2019)

data_2019$month_abb <- month.abb[data_2019$Month]
data_2019$month_abb <- factor(data_2019$month_abb, levels = month.abb)

ggplot(data_2019, aes(x = month_abb, y =Ozone)) +
  geom_boxplot() +
  xlab('') +
  ylab(expression(paste("Concentration (", mu, "g/m^3", ")"))) +
...
https://github.com/hkarnei/EDE_Fall2023
